...
if (isset($_SESSION['giohang']) && is_array($_SESSION['giohang'])) {
    ...
    foreach ($_SESSION['giohang'] as $item) {
        ...
        $html_cart .= '<tr class="cart__table--body__items">
                            ...
                            <td class="cart__table--body__list">
                                <span class="cart__price end" id="totalAmount' . number_format($i, 0) . '">' . number_format($tt,0,",",".") .'VNĐ</span>
                            </td>
                        </tr>';
        $i++;
    }
                    ...
                    <div class="col-lg-8">
                        <div class="cart__table">
                            ...
                            <div class="continue__shopping d-flex justify-content-between">
                                ...
                                <button class="cart__summary--footer__btn primary__btn cart" type="button">Cập nhật giỏ hàng</button>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4">
                        <div class="cart__summary border-radius-10">
                                ...
                                <table class="cart__summary--total__table">
                                    <tbody>
                                        <tr class="cart__summary--total__list">
                                            ...
                                            <td class="cart__summary--amount text-right"><?= number_format($sum,0,",",".")?>VNĐ</td>
                                        </tr>
                                        <tr class="cart__summary--total__list">
                            ...
                            <div class="cart__summary--footer">
                                ...
                                <ul class="d-flex justify-content-between">
                                    <!-- <li><button class="cart__summary--footer__btn primary__btn cart" type="button">Cập nhật giỏ hàng</button></li> -->
                                    <?php
                                        echo `<form action="index.php?pg=checkoutcart" method="post">
                                                ...
                                                <div class="product__variant--list quantity d-flex align-items-center mb-20">
                                                    ...
                                                    <li>
                                                        <input class="cart__summary--footer__btn primary__btn checkout" type="submit" name="btncheckout" value="Thanh Toán">
                                                    </li>
                                                </div>
                                            </form>`;
                                    ?>
...
<!-- <script>
    ...
    if (quantityWrapper) {
        quantityWrapper.forEach(function (singleItem, index) {
        let increaseButton = singleItem.querySelector(".increase");
        let decreaseButton = singleItem.querySelector(".decrease");
        let quantityInput = singleItem.querySelector(".quantity__number");
        let totalAmountElement = document.getElementById("totalAmount" + index);
        ...
        function updateTotalAmount(itemIndex, quantity) {
            // Bạn cần thay thế dòng sau với giá thực tế của mặt hàng tại chỉ mục itemIndex
            let price = parseFloat("<?= $price ?>");
            let totalAmount = quantity * price;

            // Lấy tham chiếu đến phần tử tổng số tiền
            let totalAmountElement = document.getElementById("totalAmount" + itemIndex);
            ...
            if (totalAmountElement) {
                // Định dạng giá trị và gán vào phần tử
                totalAmountElement.textContent = number_format(totalAmount, 0, ",", ".") + "VNĐ";
            }
        }
    });
    }
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        const updateCartButton = document.querySelector(".cart__summary--footer__btn.cart");
        if (updateCartButton) {
        ...
        function updateCartTotal() {
            ...
            document.querySelectorAll(".cart__table--body__items").forEach(function (item, index) {
                const quantityInput = item.querySelector(".quantity__number");
                const priceElement = item.querySelector(".cart__price");
                const totalAmountElement = item.querySelector(".cart__price.end");
                if (quantityInput && priceElement && totalAmountElement) {
                    const quantity = parseFloat(quantityInput.value);
                    const price = parseFloat(priceElement.textContent.replace(/[^\d.]/g, ''));
                    const itemTotal = quantity * price;
                    totalAmount += itemTotal;
                    // Hiển thị tổng giá trị cho từng sản phẩm
                    // totalAmountElement.textContent = itemTotal + "VND";
                    totalAmountElement.textContent = number_format(itemTotal, 0, ",", ".") + " VNĐ";
                }
            });
            // Hiển thị tổng giá trị của đơn hàng
            const totalAmountElement = document.querySelector(".cart__summary--amount.text-right");
            if (totalAmountElement) {
        ...
        return number.replace('.', dec_point).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1' + thousands_sep);
    }
    });
</script>
